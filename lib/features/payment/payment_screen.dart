...
class _PaymentScreenState extends State<PaymentScreen> {
            ...
            child: StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData &&
                  ...
                  Future.microtask(() {
                    if (mounted) {
                      setState(() => _approvedExtraCharges = snapshot.data!);
                    }
                  });
                }
                ...
                return SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // ── Order Summary card (simple top border) ──
                      AppCard(
                        margin: EdgeInsets.zero,
                        showTopBorder: true,
                        bodyPadding: const EdgeInsets.all(CSizes.lg),
                        body: _buildSummarySection(
                          isDark, charges, extrasTot, grandTot, platFee,
                        ),
                      ),
                      const SizedBox(height: CSizes.spaceBtwItems),

                      // ── Approved Extra Charges card (if any) ──
                      if (charges.isNotEmpty)
                        AppCard(
                          margin: EdgeInsets.zero,
                          showTopBorder: true,
                          bodyPadding: const EdgeInsets.all(CSizes.lg),
                          body: _buildExtrasSection(isDark, charges),
                        ),
                      if (charges.isNotEmpty)
                        const SizedBox(height: CSizes.spaceBtwItems),

                      // ── Payment Method card (simple top border) ──
                      AppCard(
                        margin: EdgeInsets.zero,
                        showTopBorder: true,
                        bodyPadding: const EdgeInsets.all(CSizes.lg),
                        body: _buildMethodSelector(isDark),
                      ),
                      const SizedBox(height: CSizes.spaceBtwItems),
  ...
  Widget _buildSummarySection(
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Inline heading (instead of header)
        Row(children: [
          Container(
        ...
        if (extrasTot > 0) ...[
          ...
          _row('Approved Extras', 'Rs. ${extrasTot.toStringAsFixed(0)}', isDark,
  ...
  // ── Extras list (body) ────────────────────────────────────
  Widget _buildExtrasSection(bool isDark, List<Map<String, dynamic>> charges) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Approved Extra Charges',
              ...
              children: [
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(desc, style: const TextStyle(fontSize: 13), maxLines: 2),
                ...
                Text('Rs. ${amt.toStringAsFixed(0)}',
                    style: const TextStyle(
                        fontWeight: FontWeight.w600, color: CColors.warning)),
              ],
            ),
          );
        }).toList(),
        ...
        children: [
          const Row(children: [
            ...
            Text('Test Mode — Use these details:',
                style: TextStyle(
                    fontWeight: FontWeight.bold, color: Colors.amber, fontSize: 13)),
          ]),
          const SizedBox(height: CSizes.sm),
        ...
        children: [
          Row(children: [
            ...
            Text('How Cash Payment Works',
                style: TextStyle(
                    fontWeight: FontWeight.bold, color: CColors.info, fontSize: 13)),
          ]),
          SizedBox(height: 10),
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            decoration: const BoxDecoration(color: CColors.info, shape: BoxShape.circle),
            child: Center(
              child: Text(num,
                  style: const TextStyle(
                      color: Colors.white,
